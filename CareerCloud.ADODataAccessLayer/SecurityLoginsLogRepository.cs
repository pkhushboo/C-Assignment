    ...
    public class SecurityLoginsLogRepository : IDataRepository<SecurityLoginsLogPoco>
    {
        public void Add(params SecurityLoginsLogPoco[] items)
        {
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["dbconnection"].ConnectionString))
            {
                ...
                cmd.Connection = conn;
                foreach (SecurityLoginsLogPoco SLLPoco in items)
                {
                    cmd.CommandText = @"INSERT INTO [dbo].[Security_Logins_Log]
                                       ([Id],[Login],[Source_IP],[Logon_Date],[Is_Succesful])
                                 VALUES
                                       (@Id, @Login, @Source_IP, @Logon_Date,@Is_Succesful)";
                    cmd.Parameters.AddWithValue("@Id", SLLPoco.Id);
                    cmd.Parameters.AddWithValue("@Login", SLLPoco.Login);
                    cmd.Parameters.AddWithValue("@Source_IP", SLLPoco.SourceIP);
                    cmd.Parameters.AddWithValue("@Logon_Date", SLLPoco.LogonDate);
                    cmd.Parameters.AddWithValue("@Is_Succesful", SLLPoco.IsSuccesful);
                    conn.Open();
        ...
        public IList<SecurityLoginsLogPoco> GetAll(params Expression<Func<SecurityLoginsLogPoco, object>>[] navigationProperties)
        {
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["dbconnection"].ConnectionString))
            {
                ...
                SecurityLoginsLogPoco[] appPocos = new SecurityLoginsLogPoco[7000];
                while (rdr.Read())
                {
