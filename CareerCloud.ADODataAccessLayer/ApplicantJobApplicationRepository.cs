    ...
    public class ApplicantJobApplicationRepository : IDataRepository<ApplicantJobApplicationPoco>
    {
        public void Add(params ApplicantJobApplicationPoco[] items)
        {
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["dbconnection"].ConnectionString))
            {
                ...
                cmd.Connection = conn;
                foreach (ApplicantJobApplicationPoco AJAPoco in items)
                {
                    cmd.CommandText = @"INSERT INTO [dbo].[Applicant_Job_Applications]
                                      ([Id],[Applicant],[Job],[Application_Date])
                                 VALUES
                                     (@Id,@Applicant,@Job,@Application_Date)";
                    cmd.Parameters.AddWithValue("@Id", AJAPoco.Id);
                    cmd.Parameters.AddWithValue("@Applicant", AJAPoco.Applicant);
                    cmd.Parameters.AddWithValue("@Job", AJAPoco.Job);
                    cmd.Parameters.AddWithValue("@Application_Date", AJAPoco.ApplicationDate);
                    conn.Open();
        ...
        public void Update(params ApplicantJobApplicationPoco[] items)
        {
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["dbconnection"].ConnectionString))
            {
                foreach (ApplicantJobApplicationPoco AEPoco in items)
                {
                    SqlCommand cmd = new SqlCommand(@"UPDATE Applicant_Job_Applications
                                                SET 
                                                  Applicant = @Applicant, Job = @Job,Application_Date = @Application_Date
                                                WHERE Id =@Id", conn);
                    cmd.Parameters.AddWithValue("@Id", AEPoco.Id);
                    cmd.Parameters.AddWithValue("@Applicant", AEPoco.Applicant);
                    cmd.Parameters.AddWithValue("@Job", AEPoco.Job);
                    cmd.Parameters.AddWithValue("@Application_Date", AEPoco.ApplicationDate);
                    conn.Open();
                    cmd.ExecuteNonQuery();
                    conn.Close();
                }
