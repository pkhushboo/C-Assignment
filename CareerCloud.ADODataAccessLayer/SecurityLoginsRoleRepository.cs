    ...
    public class SecurityLoginsRoleRepository : IDataRepository<SecurityLoginsRolePoco>
    {
        public void Add(params SecurityLoginsRolePoco[] items)
        {
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["dbconnection"].ConnectionString))
            {
                ...
                cmd.Connection = conn;
                foreach (SecurityLoginsRolePoco SLRPoco in items)
                {
                    cmd.CommandText = @"INSERT INTO [dbo].[Security_Logins_Roles]
                                        ([Id],[Login],[Role])
                                   VALUES
                                        (@Id,@Login,@Role)";
                    cmd.Parameters.AddWithValue("@Id", SLRPoco.Id);
                    cmd.Parameters.AddWithValue("@Login", SLRPoco.Login);
                    cmd.Parameters.AddWithValue("@Role", SLRPoco.Role);
                    conn.Open();
